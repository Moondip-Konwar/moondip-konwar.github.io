...
{#if modalOpen && currentProject}
	<div class="modal-overlay" on:click={close}>
		<div class="modal-box card" on:click={clickInside}>
			<h2 class="neon-green mb-3 text-xl font-bold">{currentProject.title}</h2>
    ...
    transition: color 0.25s ease;
			...
			<div class="mt-5 flex justify-end gap-3">
				...
				<button class="modal-btn" on:click={close}> Close </button>
			</div>
		</div>
	</div>
{/if}
...
<style>
	...
	.modal-box {
		...
		animation: slideUp 0.25s cubic-bezier(0.16, 0.9, 0.2, 1);
	}
